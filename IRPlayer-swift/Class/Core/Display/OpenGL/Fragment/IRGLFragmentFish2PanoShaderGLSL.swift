//
//  IRGLFragmentFish2PanoShaderGLSL.swift
//  IRPlayer-swift
//
//  Created by irons on 2024/9/14.
//
...
class IRGLFragmentFish2PanoShaderGLSL {
    ...
    static func PIXEL_FORMAT_TOKEN(pixelFormat: IRPixelFormat) -> String {
        switch pixelFormat {
        case .RGB_IRPixelFormat:
            return "uniform highp sampler2D s_texture;"
        case .YUV_IRPixelFormat:
            return "uniform highp sampler2D s_texture_y; uniform highp sampler2D s_texture_u; uniform highp sampler2D s_texture_v;"
        case .NV12_IRPixelFormat:
            return "uniform highp sampler2D SamplerY; uniform highp sampler2D SamplerUV;"
        }
    }

    static func PIXEL_FORMAT_PROCESSING(pixelFormat: IRPixelFormat) -> String {
        switch pixelFormat {
        case .RGB_IRPixelFormat:
            return "vecTmp = texture2D(SamplerY, vec2(float(u)/float(vars.fishwidth), float(v)/float(vars.fishheight)));"
        case .YUV_IRPixelFormat:
            return "vecTmp.x = texture2D(s_texture_y, vec2(float(u)/float(vars.fishwidth), float(v)/float(vars.fishheight))).r - (16.0/255.0); vecTmp.y = texture2D(s_texture_u, vec2(float(u)/float(vars.fishwidth), float(v)/float(vars.fishheight))).r - 0.5; vecTmp.z = texture2D(s_texture_v, vec2(float(u)/float(vars.fishwidth), float(v)/float(vars.fishheight))).r - 0.5; vecTmp = colorConversionMatrix * vecTmp;"
        case .NV12_IRPixelFormat:
            return "vecTmp.x = (texture2D(SamplerY, vec2(float(u)/float(vars.fishwidth), float(v)/float(vars.fishheight))).r - (16.0/255.0))* lumaThreshold; vecTmp.yz = (texture2D(SamplerUV, vec2(float(u)/float(vars.fishwidth), float(v)/float(vars.fishheight))).rg - vec2(0.5, 0.5))* chromaThreshold; vecTmp = colorConversionMatrix * vecTmp;"
        }
    }

    static func getShaderString(pixelFormat: IRPixelFormat, antialias: Int) -> String {

        var declareTexUVs: String = {
            var setTex = ""
            let texNum = antialias * antialias
            for i in 0..<texNum {
                setTex += "uniform highp sampler2D texUV\(i);"
            }
            return setTex
        }()
        var useTexUVs: String = {
            var getTex = ""
            let texNum = antialias * antialias
            for i in 0..<texNum {
                getTex += "\(i == 0 ? "" : "else ")if(aa == \(i)) { rgba = texture2D(texUV\(i), xy); }"
            }
            return getTex
        }()

        var fish2panoFragmentShaderString = """
        ...
        \(PIXEL_FORMAT_TOKEN(pixelFormat: pixelFormat))
                ...
                for (ai=0;ai<vars.antialias;ai++) {
                    for (aj=0;aj<vars.antialias;aj++) {
                        if (FindFishPixel(ai*vars.antialias+aj,v_texcoord,u,v) > 0) {
                            mediump vec3 vecTmp;
                            \(PIXEL_FORMAT_PROCESSING(pixelFormat: pixelFormat))
