//
//  IRGLFragmentFish2PerspShaderGLSL.swift
//  IRPlayer-swift
//
...
class IRGLFragmentFish2PerspShaderGLSL {
    ...
    static func PIXEL_FORMAT_PROCESSING(pixelFormat: IRPixelFormat) -> String {
        switch pixelFormat {
        case .RGB_IRPixelFormat:
            return "vecTmp.x = (texture2D(SamplerY, vec2(float(u)/float(fishwidth), float(v)/float(fishheight))).r - (16.0/255.0))* lumaThreshold; vecTmp.yz = (texture2D(SamplerUV, vec2(float(u)/float(fishwidth), float(v)/float(fishheight))).rg - vec2(0.5, 0.5))* chromaThreshold; vecTmp = colorConversionMatrix * vecTmp;"
        case .YUV_IRPixelFormat:
    ...
    static func getShaderString(pixelFormat: IRPixelFormat) -> String {

        var fish2perspFragmentShaderString = """
