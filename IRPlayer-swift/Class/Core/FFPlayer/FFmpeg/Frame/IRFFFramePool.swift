...
class IRFFFramePool: NSObject, IRFFFrameDelegate {
    ...
    static func videoPool() -> IRFFFramePool {
        return IRFFFramePool(capacity: 60, frameClassName: NSClassFromString("IRFFAVYUVVideoFrame")!)
    }

    static func audioPool() -> IRFFFramePool {
        return IRFFFramePool(capacity: 500, frameClassName: NSClassFromString("IRPlayer_swift.IRFFAudioFrame")!)
    }
